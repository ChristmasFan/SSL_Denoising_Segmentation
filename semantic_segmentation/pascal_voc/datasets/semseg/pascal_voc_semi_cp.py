...
def build_voc_semi_loader_cp(split, all_cfg, seed=0):
    ...
    if acp:
        ...
        paste_trs = None
    dset = voc_dset(cfg['data_root'], cfg['data_list'], trs_form, seed, n_sup, split, acp=acp, paste_trs=paste_trs, prob=prob)
    ...
    dset_unsup = voc_dset(cfg['data_root'], cfg['data_list'], trs_form_unsup, seed, n_sup, split, unsup=True, fm=fm, acm=acm)
    if split == 'train':
